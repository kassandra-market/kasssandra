...
fn setup_with_curve(threshold: u64, cap: u64, max: u64) -> (TestCtx, Pubkey, Keypair) {
    ...
    let (_config, res) =
        ctx.init_config_with_curve(authority.pubkey(), base, BASE, 100, fee_destination, threshold, cap, max);
    assert!(res.is_ok(), "{res:?}");
    (ctx, base, authority)
}

fn create_one(ctx: &mut TestCtx, base: Pubkey) -> Market {
...
fn floor_stays_at_base_while_disabled_max_equals_base() {
    ...
    let (mut ctx, base, _auth) = setup_with_curve(0, 1_000_000_000, BASE); // max == base ⇒ disabled
    for _ in 0..5 {
        let m = create_one(&mut ctx, base);
...
fn floor_ramps_up_with_rapid_creation_and_first_market_sees_no_prior_activity() {
    ...
    let (mut ctx, base, _auth) = setup_with_curve(0, 3 * one_unit, MAX);
    ...
    let m1 = create_one(&mut ctx, base);
    ...
    let m2 = create_one(&mut ctx, base);
    let m3 = create_one(&mut ctx, base);
    let m4 = create_one(&mut ctx, base); // at/above cap ⇒ pinned at MAX
...
fn floor_decays_back_down_after_idle_time() {
    ...
    let _m1 = create_one(&mut ctx, base);
    let m2 = create_one(&mut ctx, base); // ramped up from m1's activity
    ...
    let m4 = create_one(&mut ctx, base);
...
fn in_flight_markets_are_immune_to_a_later_governance_curve_change() {
    let (mut ctx, base, authority) = setup_with_curve(0, 1_000_000_000, BASE); // disabled
    let m_before = create_one(&mut ctx, base);
    ...
    let m_after = create_one(&mut ctx, base);
