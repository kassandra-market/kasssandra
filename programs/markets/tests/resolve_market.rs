...
/// `activate`. Returns the context, SOL mint, market PDA, oracle, MetaDAO refs.
...
/// Create a test user who holds ONLY cYES: split `amount` SOL into cYES+cNO, then
/// transfer the cNO leg out to a sink so the user's cNO balance is 0. Returns the
/// user keypair and its (base_ata, cyes, cno) accounts.
...
/// Mirror of [`holder_of_yes_only`] for the LOSING leg: split `amount` SOL then
...
/// Base context for a 3-option categorical oracle: SVM + MetaDAO loaded, SOL
/// mint, config, and a 3-option oracle in Proposal. Returns (ctx, base, oracle).
...
pub(crate) fn activate_sub_market(
    ...
    let creator_ata = ctx.create_token_account(base, creator.pubkey(), 5_000_000_000);
    let (market, res) =
        ctx.create_market_full(&creator, oracle, base, creator_ata, MIN_LIQ, outcome_index);
    assert!(res.is_ok(), "create sub-market {outcome_index}: {res:?}");
    let refs = ctx.compose_metadao_market(market, oracle, base);
    let res = ctx.activate_at(oracle, base, outcome_index);
