...
use kassandra_markets_program::state::{AccountType, Config, MAX_FEE_BPS};
use solana_sdk::{
    pubkey::Pubkey,
...
fn init_config_happy_path() {
    ...
    let fee_destination = ctx.create_token_account(base, authority, 0);
    let (cfg_pda, res) = ctx.init_config_full(authority, base, 1_000_000_000, 250, fee_destination);
    ...
    assert_eq!(c.base_mint.to_bytes(), base.to_bytes());
...
fn init_config_rejects_wrong_fee_mint() {
    // A fee_destination token account on a mint other than SOL must be rejected.
    let mut ctx = TestCtx::new();
    let base = ctx.create_mint(9);
    let other = ctx.create_mint(9);
    let authority = Keypair::new().pubkey();
    let bad_destination = ctx.create_token_account(other, authority, 0);
    let (_pda, res) = ctx.init_config_full(authority, base, 1_000_000_000, 100, bad_destination);
...
fn init_config_twice_fails() {
    ...
    let base = ctx.create_mint(9);
    let authority = Keypair::new().pubkey();
    let (_pda, res1) = ctx.init_config(authority, base, 1);
    assert!(res1.is_ok());
    ...
    let (_pda, res2) = ctx.init_config(authority, base, 2);
    assert_eq!(
        custom_code(&res2),
...
fn init_config_rejects_non_upgrade_authority() {
    ...
    let (_pda, ok) = ctx.init_config_full(authority, base, 1_000_000_000, 100, fee_destination);
    assert!(ok.is_ok(), "{ok:?}");
}
