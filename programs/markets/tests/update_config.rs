...
use kassandra_markets_program::state::{Config, MAX_FEE_BPS};
...
fn update_config_by_authority_ok() {
    ...
    let (cfg, _) = ctx.init_config(authority.pubkey(), base, 100);
...
fn update_config_by_stranger_fails() {
    ...
    let base = ctx.create_mint(9);
    let authority = Keypair::new();
    let _ = ctx.init_config(authority.pubkey(), base, 100);
    let stranger = Keypair::new();
    ctx.svm_airdrop(&stranger.pubkey());
    let res = ctx.update_config(&stranger, base, 999);
...
fn update_config_rejects_wrong_fee_mint() {
    // A fee_destination on a mint other than the config's SOL mint is rejected.
    let mut ctx = TestCtx::new();
    let base = ctx.create_mint(9);
