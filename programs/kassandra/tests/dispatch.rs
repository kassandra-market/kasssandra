...
fn known_discriminant_returns_not_implemented() {
    ...
    // (SubmitFact/VoteFact/FinalizeFacts/AdvancePhase are implemented; use a
    // still-stubbed instruction here.)
    let ix = ix_with_data(&ctx, vec![Ix::SubmitAiClaim as u8, 0xAB]);
    let err = ctx.send(ix, &[]).unwrap_err().err;
    assert_eq!(
        err,
