...
fn admin_sets_governance_records_linkage_and_defaults() {
    ...
    assert_eq!(p0.spot_dao, [0u8; 32].into());
    ...
    let (spot_dao, dao_authority) = ctx.fabricate_dao_and_vault();
    let payer = ctx.payer.insecure_clone();
    let (_pda, res) = ctx.set_governance(&payer, dao_authority, spot_dao);
    ...
    assert_eq!(p.spot_dao, spot_dao.to_bytes().into());
...
fn non_admin_cannot_set_governance() {
    ...
    let (dao_authority, spot_dao) = TestCtx::stand_in_governance(0xBB);
    let (_pda, res) = ctx.set_governance(&stranger, dao_authority, spot_dao);
    assert_eq!(
        custom_code(&res),
...
fn handoff_is_one_shot_admin_rejected_after_handoff() {
    ...
    let (spot_dao, vault) = ctx.fabricate_dao_and_vault();
    let payer = ctx.payer.insecure_clone();
    let (_pda, res) = ctx.set_governance(&payer, vault, spot_dao);
    ...
    let (spot_dao2, vault2) = ctx.fabricate_dao_and_vault();
    let (_pda, res) = ctx.set_governance(&payer, vault2, spot_dao2);
    assert_eq!(
        custom_code(&res),
...
//   - a `spot_dao` not owned by the futarchy program  -> InvalidFutarchyDao
//   - a futarchy-owned `spot_dao` with a bad discriminator -> InvalidFutarchyDao
//   - a `dao_authority` that is NOT the derived Squads vault -> DaoAuthorityMismatch
// ─────────────────────────────────────────────────────────────────────────────

#[test]
fn rejects_non_futarchy_owned_spot_dao() {
    let mut ctx = TestCtx::new();
    let (_pda, res) = ctx.init_protocol();
    assert!(res.is_ok());
    ...
    let dao_authority = TestCtx::squads_vault_for_dao(&spot_dao);
    ...
    assert_eq!(
        ...
        "a spot_dao not owned by the futarchy program must be rejected: {res:?}"
    );
}

#[test]
fn rejects_spot_dao_with_bad_discriminator() {
    let mut ctx = TestCtx::new();
    let (_pda, res) = ctx.init_protocol();
    assert!(res.is_ok());
    ...
    ctx.fabricate_owned_account(spot_dao, owner, blob);
    let dao_authority = TestCtx::squads_vault_for_dao(&spot_dao);
    ...
    assert_eq!(
        custom_code(&res),
        Some(KassandraError::InvalidFutarchyDao as u32),
        "a spot_dao with the wrong Anchor discriminator must be rejected: {res:?}"
    );
}
...
fn rejects_dao_authority_that_is_not_derived_vault() {
    ...
    let (spot_dao, _correct_vault) = ctx.fabricate_dao_and_vault();
    let wrong_authority = Pubkey::new_unique();

    let payer = ctx.payer.insecure_clone();
    let (_pda, res) = ctx.set_governance(&payer, wrong_authority, spot_dao);
    assert_eq!(
        custom_code(&res),
...
fn zero_linkage_keys_rejected() {
    ...
    assert_eq!(
        custom_code(&res),
        Some(KassandraError::InvalidAccount as u32),
        "zero spot_dao must be rejected: {res:?}"
    );
}
