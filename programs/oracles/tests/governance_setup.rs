...
fn admin_sets_governance_records_linkage_and_defaults() {
    ...
    // Emission is ON by default (the recommended curve): each create_oracle mints
    // a reward. Farming is throttled by the creation-fee recapture, not by
    // disabling emission (see `crate::fee::creation_fee`).
