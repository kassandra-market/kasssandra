...
pub(crate) struct MarketAccounts {
    ...
    pub(crate) pass_mint: Pubkey, // pass-SOL  (cond(base_vault, 0))
    pub(crate) fail_mint: Pubkey, // fail-SOL  (cond(base_vault, 1))
    pub(crate) pass_usdc: Pubkey, // pass-USDC  (cond(usdc_vault, 0))
    pub(crate) fail_usdc: Pubkey, // fail-USDC  (cond(usdc_vault, 1))
}

/// Compose the MetaDAO market (binary question + SOL/USDC conditional vaults)
/// for `resolver` and return the bound mints/vaults plus the oracle-PDA-owned
/// conditional-SOL destinations for the proposer's split bond.
