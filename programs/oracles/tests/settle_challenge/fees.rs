...
fn settle_flip_slashed_then_disqualified_no_underflow() {
    ...
    // The carve-out tops the prior slash up to bond − base_fee WITHOUT underflow
    // (defaults: 50% flip + 1% fee = 51% ≤ 100%), and the base_fee is capped to the
    ...
    let net_slash = BOND - base_fee; // 0.99 bond ≥ the 0.5 prior slash
    let p = ctx.proposer(f.proposer);
    assert_eq!(p.disqualified, 1);
    assert_eq!(p.slashed_amount, net_slash, "topped up to bond − base_fee");
...
fn settle_fee_rates_are_oracle_snapshotted() {
    ...
    // NOT a hard-coded const. Retune the snapshot to 5% SOL / 2% USDC and assert
    // the disqualify-path SOL fee tracks it. (set_config → new-oracle snapshot is
    // covered in set_config.rs; this pins the settle-side consumption.)
    let (mut ctx, f) = fixture(QUOTE_LOW, QUOTE_HIGH); // fraud (disqualify) path
                                                       // 5% SOL fee on a successful challenge, 2% USDC fee on a failed one.
    ...
    assert_eq!(ctx.proposer(f.proposer).slashed_amount, BOND - base_fee);
    assert_eq!(
        ctx.oracle(f.oracle).bond_pool,
        bond_pool_before + BOND - base_fee
    );
