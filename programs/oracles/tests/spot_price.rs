//! Tests for `spot_price` (Task F5): the governance-anchored SOL/USDC spot TWAP
//! read from the canonical futarchy `Dao` account (`Protocol.spot_dao`).
//!
...
//! the futarchy program, with its key recorded as `Protocol.spot_dao` via
...
/// `spot_dao` as the given DAO account key. Returns the Protocol PDA.
fn governed_ctx(ctx: &mut TestCtx, spot_dao: Pubkey) -> Pubkey {
    ctx.ensure_protocol();
    // The spot_price read only consults `spot_dao`; record the linkage directly
...
#[test]
fn reads_spot_twap_from_spot_dao() {
    ...
    // Fabricate the canonical futarchy Dao account and bless it as `spot_dao`.
    ...
    let protocol = governed_ctx(&mut ctx, spot_dao);

    let ix = ctx.spot_price_ix(protocol, spot_dao);
    let res = ctx.send(ix, &[]);
    assert!(res.is_ok(), "spot_price should succeed: {res:?}");
...
fn wrong_account_rejected() {
    ...
    // governance anchor (`key == Protocol.spot_dao`) must reject it.
    ...
    let ix = ctx.spot_price_ix(protocol, impostor);
    let res = ctx.send(ix, &[]);
    assert_eq!(
        custom_code(&res),
        Some(KassandraError::InvalidAccount as u32),
        "an account != Protocol.spot_dao must be rejected: {res:?}",
    );
}
...
fn non_futarchy_owner_rejected() {
    ...
    // The blessed `spot_dao` carries a valid oracle blob, but is owned by the
...
fn substituted_protocol_rejected() {
    ...
    let _protocol = governed_ctx(&mut ctx, spot_dao);
    ...
    let ix = ctx.spot_price_ix(fake_protocol, spot_dao);
    let res = ctx.send(ix, &[]);
    assert_eq!(
        custom_code(&res),
        Some(KassandraError::InvalidAccount as u32),
