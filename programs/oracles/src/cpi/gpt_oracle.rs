...
/// LLM context PDA seed prefix (`["test-context", counter]`).
...
pub const INTERACT_IX_MAX: usize =
    8 + 4 + MAX_INTERACT_TEXT + 32 + 8 + 1 + 4 + (3 * 34);
...
fn parse_json_u8_field(haystack: &[u8], key: &[u8]) -> Option<u8> {
    ...
    while i + key.len() < haystack.len() {
        if &haystack[i..i + key.len()] == key {
            let mut j = i + key.len();
            while j < haystack.len() && (haystack[j] == b' ' || haystack[j] == b'\t' || haystack[j] == b'"')
            {
                j += 1;
            }
...
mod tests {
    ...
    fn parse_option_index_json() {
        ...
        assert_eq!(parse_llm_option(br#"{"option_index":1,"reason":"x"}"#, 3).unwrap(), 1);
