...
pub fn process(program_id: &Pubkey, accounts: &mut [AccountInfo], payload: &[u8]) -> ProgramResult {
    ...
    let (pass_base_mint, _) = metadao::conditional_token_mint_pda(&market.base_vault, 0);
    let (fail_base_mint, _) = metadao::conditional_token_mint_pda(&market.base_vault, 1);
    ...
    let pass_twap = verify_and_read_twap(pass_amm_ai, &pass_base_mint, &pass_usdc_mint)?;
    let fail_twap = verify_and_read_twap(fail_amm_ai, &fail_base_mint, &fail_usdc_mint)?;
    ...
    // The redeem + fee CPIs below need: the stake vault (redeem dest + SOL-fee
    // source), the SOL conditional vault + its underlying ATA, the conditional
    // SOL mints + the oracle-PDA-owned holders the bond was split into, the USDC
    ...
    assert_key(base_vault_ai, &market.base_vault)?;
    assert_key(pass_base_mint_ai, &pass_base_mint)?;
    assert_key(fail_base_mint_ai, &fail_base_mint)?;
    assert_key(oracle_pass_base_ai, &market.oracle_pass_base)?;
    assert_key(oracle_fail_base_ai, &market.oracle_fail_base)?;
    ...
    assert_owned_by_program(base_vault_ai, &metadao::CONDITIONAL_VAULT_ID)?;
    {
        ...
        if &v_underlying_acct != base_vault_underlying_ai.address() {
            return Err(KassandraError::InvalidAccount.into());
        }
    }
    ...
    // Directional SOL fee on a SUCCESSFUL challenge (disqualify): a carve-out of
    // the bond to the challenger. The proposer's `bond_pool` contribution becomes
    // `bond − base_fee` (NOT the full bond), keeping the per-proposer identity
    ...
    // carve-out tops that up to `bond − base_fee`, which must stay ≥ the prior
    // slash. The `set_config` joint bound (`flip_slash_frac + success_base_fee_frac
    ...
    // / brick settlement, nor transfer more SOL than is left in stake_vault. The
    // capped value drives BOTH the accounting and the SOL transfer below.
    let remaining_bond = proposer.bond.saturating_sub(proposer.slashed_amount);
    let base_fee = fee_amount(
        proposer.bond,
        oracle.challenge_success_base_fee_num,
        oracle.challenge_success_base_fee_den,
    )?
    .min(remaining_bond);

    if disqualify && !proposer.is_disqualified() {
        // Net slash = bond − base_fee (≥ slashed_amount by the cap). Top up any
        // prior (flip) slash to exactly that net (never double-counting, never
        // exceeding the escrowed bond): the base_fee leaves to the challenger
    ...
    let redeem_metas = [
        ...
        InstructionAccount::readonly(cv_event_auth_ai.address()),
        InstructionAccount::readonly(cv_prog_ai.address()),
        InstructionAccount::writable(pass_base_mint_ai.address()),
        InstructionAccount::writable(fail_base_mint_ai.address()),
