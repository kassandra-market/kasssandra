...
//! #1, NOT a futarchy PDA) and `spot_dao` (the futarchy `Dao` account whose
//! embedded spot AMM is F5's SOL price source) into the [`Protocol`] singleton.
...
//! - The `spot_dao` ACCOUNT is threaded (read-only). Its key must equal the
//!   payload `spot_dao`, it must be owned by the futarchy program
...
//!   derived for that DAO (multisig `create_key == spot_dao` → multisig → vault,
...
//!
//! # Mint authority
//! The SOL mint authority is the program PDA `[b"mint_authority"]` (see
...
//! 2. spot_dao     — read-only; the futarchy `Dao` account (must equal the
//!    payload `spot_dao`; validated owner + discriminator)
//!
//! # Instruction payload
//! `dao_authority: [u8; 32]` ++ `spot_dao: [u8; 32]` (64 bytes).
...
pub fn process(program_id: &Pubkey, accounts: &mut [AccountInfo], payload: &[u8]) -> ProgramResult {
    let [protocol_ai, authority_ai, spot_dao_ai, ..] = accounts else {
    ...
    let spot_dao: Pubkey = <[u8; 32]>::try_from(&payload[32..64]).unwrap().into();
    // Both linkage keys must be non-zero (a zeroed key is the "unset" sentinel).
    if dao_authority == Pubkey::default() || spot_dao == Pubkey::default() {
    ...
    // (a) The passed `spot_dao` account must be the one named in the payload.
    assert_key(spot_dao_ai, &spot_dao)?;
    ...
    if !spot_dao_ai.owned_by(&md6::FUTARCHY_ID) {
        return Err(KassandraError::InvalidFutarchyDao.into());
    }
    {
        let data = spot_dao_ai.try_borrow()?;
    ...
    //     this DAO (multisig `create_key == spot_dao` → multisig → vault, idx 0).
    let (multisig, _) = md6::squads_multisig_pda(&spot_dao);
