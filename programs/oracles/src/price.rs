//! `spot_price` — governance-anchored SOL/USDC spot TWAP read (Task F5).
//!
...
//! The price is read **directly from the `Protocol.spot_dao` account's bytes** —
//! there is NO separate pool account to bind. `spot_dao` IS the futarchy `Dao`
//! account, recorded by `set_governance` (F1), and the embedded spot oracle is at
//! the fixed offsets above. So the "binding from `spot_dao` to the spot-oracle
...
//!    `Protocol.spot_dao` (the DAO blessed by the one-shot `set_governance`
...
//!    documents intent. (Before `set_governance`, `spot_dao` is the zero pubkey
...
//! buffer. `spot_price` propagates that unchanged.
//!
//! # No on-chain consumer yet
//! This ships as a validated primitive. The challenge-market rework (next
//! milestone) is its first consumer; the optional [`crate::instruction::Ix::SpotPrice`]
...
/// Read the SOL/USDC spot TWAP from the governance-blessed futarchy `Dao`
/// account, scaled by `1e12` (quote units per base unit; see
/// [`metadao_v06::futarchy_spot_twap`]).
///
/// `spot_dao_ai` must be the account whose key equals `protocol.spot_dao` and
...
pub fn spot_price(protocol: &Protocol, spot_dao_ai: &AccountInfo) -> Result<u128, ProgramError> {
    // (1) Governance anchor: only the DAO recorded by `set_governance`.
    assert_key(spot_dao_ai, &protocol.spot_dao)?;
    // (2) Owner anchor (defense-in-depth): it must be a futarchy `Dao` account.
    assert_owned_by_program(spot_dao_ai, &metadao_v06::FUTARCHY_ID)?;
