...
function WalletRow({
  ...
  return (
    <button
      ...
      className="flex items-center gap-3 rounded-button border border-hairline bg-liquid-kelp px-4 py-3 font-inter text-[14px] text-platinum hover:bg-[#04524c] disabled:opacity-60"
    >
      {wallet.icon ? <img src={wallet.icon} alt="" className="h-5 w-5 rounded" /> : null}
      <span className="flex-1 text-left">{wallet.name}</span>
      {isConnecting ? <span className="text-silver">Connecting…</span> : null}
    </button>
  )
}
...
export function WalletMenu() {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm rounded-card border border-hairline bg-liquid-deep p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="font-serif text-heading-sm font-light text-platinum">Connect a wallet</h2>
        <div className="mt-4 flex flex-col gap-2">
          {connectable.length === 0 ? (
            <p className="font-inter text-[14px] text-silver">
