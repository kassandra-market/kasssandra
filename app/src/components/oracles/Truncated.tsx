...
import { truncateMiddle } from '../../lib/oracleView'

export interface TruncatedProps {
  /** The full value (pubkey / hash). Displayed truncated; copied in full. */
  value: string
  ...
  copyable?: boolean
  /** Accessible noun for the value, e.g. "oracle address" — used in the copy label. */
  label?: string
  className?: string
}

/**
 * A reusable truncated identifier. Pubkeys/hashes are shown as `Abc1…Xy9z` in
 * monospace; when `copyable`, the whole thing is a labelled button that writes
 * the full value to the clipboard and flashes a transient confirmation. No
 * ember here — the copy affordance stays sepia to protect the ember budget.
 */
