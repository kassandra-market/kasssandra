...
/** The SOL token mark — a circular aqua coin badge beside the symbol. */
function KassBadge() {
  return (
    <span
...
/**
 * Add SOL liquidity to an ACTIVE market's live cYES/cNO AMM. The deposit is split
 ...
 *
 * Presented as a pool-deposit ticket: a large token-amount row (SOL mark + symbol
  ...
  const body = (
    <>
      <div>
        ...
        >
          Deposit SOL into the live pool. It's split into YES/NO and added at the current price;
      ...
      <ConnectGate connected={action.connected}>
        <form className="flex flex-col gap-3" onSubmit={onSubmit} noValidate>
          ...
          {/* Token-amount row — SOL mark + symbol on the left, the amount on the right. */}
          <div
          ...
          >
            <KassBadge />
            <span className="font-serif text-heading-sm font-light text-platinum">SOL</span>
            <input
              inputMode="decimal"
              placeholder="0.00"
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className="font-inter text-[12px] text-silver">
              ...
              <span className="tabular-nums text-silver">
                {balance === null ? (balanceLoading ? "…" : "—") : formatSol(balance)}
              </span>
            </span>
