/**
 * Best-effort SOL→USD price read for the trade UI's unit toggle.
 *
 * The manipulation-resistant SOL/USDC price the protocol governs around is the
 * futarchy program's embedded spot `TwapOracle`, which lives INSIDE the
 * `Protocol.spot_dao` account (recorded by `set_governance`). Mirrors the on-chain
 ...
 *
 * a `PRICE_SCALE` (1e12) scaled quote-per-base price (USDC raw units per SOL raw
...
import { SOL_DECIMALS, USDC_DECIMALS } from "../lib/oracleView";
...
/** The all-ones system pubkey — `spot_dao` reads this until governance is set. */
 ...
 * whole SOL: divide out `PRICE_SCALE`, then correct for the base/quote decimal
 * scales (`10^(SOL_DECIMALS − USDC_DECIMALS)`).
 */
export function usdcPerSol(twap: bigint): number {
  return (Number(twap) / Number(PRICE_SCALE)) * 10 ** (SOL_DECIMALS - USDC_DECIMALS);
}

/**
 * Fetch the current SOL→USD (USDC) price, or `null` when it isn't available:
 * the Protocol singleton is missing, governance isn't linked (`spot_dao` unset),
 ...
 */
export async function fetchSolUsdcPrice(conn: Connection): Promise<number | null> {
  try {
    ...
    if (!protocol.governanceSet || protocol.spotDao.toString() === ZERO_ADDRESS) return null;
    const daoInfo = await conn.getAccountInfo(protocol.spotDao);
    if (!daoInfo || daoInfo.data.length === 0) return null;
    const twap = decodeFutarchySpotTwap(daoInfo.data);
    if (twap === null) return null;
    const price = usdcPerSol(twap);
