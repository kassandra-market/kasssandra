 ...
 * the LP they added — not read as a "0 SOL contribution". A pure funder still
 * reads as their SOL stake.
 */
...
describe("ClaimLpControl position copy", () => {
  it("describes the added LP for a pure late LP (not 0 SOL)", () => {
  ...
  it("describes the SOL stake for a pure funder", () => {
    state.address = "Fund1111";
    const html = render(true, [contribution("Fund1111", 1_000_000_000n, 0n)]);
    expect(html).toContain("1 SOL funding contribution");
  ...
  it("describes both for a funder who also added liquidity", () => {
    ...
    expect(html).toContain("2 SOL funding contribution and 3 LP you added to the pool");
  });
});
