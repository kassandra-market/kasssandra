 ...
 * row (SOL mark + symbol), a balance line with percentage quick-sets, a risk
...
vi.mock("../src/market/hooks/useSolBalance", () => ({
  useSolBalance: () => ({ balance: state.balance, loading: false, refetch: () => {} }),
}));
...
const market = { baseMint: { toString: () => "Kass1111" } } as never;
...
describe("AddLiquidityControl — deposit ticket", () => {
  it("renders the token-amount row, balance, and percentage quick-sets", () => {
    ...
    // Balance line surfaces the wallet balance (12 SOL).
