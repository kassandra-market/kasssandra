...
test('funded wallet connects and the seeded oracles load from the local validator', async ({
  ...
  // `/oracles` redirects to the unified `/markets` list.
  await expect(page).toHaveURL(/\/markets/)

  // The combined stats strip only renders once the oracle list has been fetched
  // from surfpool — its presence proves the app is talking to the local chain.
  await expect(page.getByLabel('Capital at stake')).toBeVisible()
