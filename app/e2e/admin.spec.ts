...
test('setGovernance: admin hands the DAO linkage to the wallet', async ({ page }) => {
  ...
  await page.goto(`/admin?spotDao=${wallet.spotDao}`)
  await expect(page.getByRole('button', { name: /^Connected:/ })).toBeVisible()
  await page.getByRole('button', { name: /Set governance/i }).click()
  // On-chain: governance_set flips 0 → 1 and spot_dao is recorded.
  const p = await poll(() => protocolAt(wallet.protocol), (x) => x.governanceSet === true)
  expect(p.spotDao.toString()).toBe(wallet.spotDao)
})
...
test('spotPrice: reads the SOL spot TWAP from the futarchy DAO', async ({ page }) => {
  // Permissionless read — ensure Protocol.spot_dao points at the fabricated DAO.
  await patchProtocol(wallet.protocol, { spotDao: wallet.spotDao })
  await page.goto(`/admin?spotDao=${wallet.spotDao}`)
  await expect(page.getByRole('button', { name: /^Connected:/ })).toBeVisible()
  await page.getByRole('button', { name: /Read SOL price/i }).click()
