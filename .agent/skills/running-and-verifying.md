...
```bash
just build                              # rebuild the markets .so (do this before any test that loads a program)
cargo build --workspace                 # fast compile check
cargo clippy --workspace --all-targets  # lint — introduce NO new warnings vs baseline
cargo test --workspace                  # preferred way to run tests
```
...
- **Rebuild `.so` before tests** — LiteSVM `include_bytes!`s them; a stale `.so`
  ...
  instruction data").
- Clippy: introduce **no new warnings**. The oracles program and in-house
  runner crates are gone; do not revive their old baseline.
...
```bash
pnpm --filter @kassandra-market/markets build      # tsc → dist
pnpm --filter @kassandra-market/markets typecheck  # tsc --noEmit
pnpm --filter @kassandra-market/markets test       # vitest (litesvm + decoders)
# GPT surfpool e2e: KASSANDRA_MARKET_E2E=1 (needs surfpool + tracked ELF + llm_oracle)
```
...
pnpm --filter ./app exec vitest run      # unit + render tests
