...
accounts. The client composes the MetaDAO `Question`, SOL `conditional_vault`, and
...
<Steps>
  ...
  <Step title="SOL conditional vault">
    Re-derived from the question + SOL mint; its `question`, `underlying_mint`
    (== `market.base_mint`), and `underlying_account` must all match.
  </Step>
...
```mermaid
flowchart LR
  E[escrow SOL<br/>total_contributed] -->|split_tokens| Y[cYES] & N[cNO]
