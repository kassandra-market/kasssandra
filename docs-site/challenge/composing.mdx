...
<Steps>
  ...
  <Step title="Create SOL vault — conditional_vault::initialize_conditional_vault">
    A vault with underlying = `oracle.baseMint`. Mints the conditional-token mints:
    idx 0 = **pass-SOL**, idx 1 = **fail-SOL**. SDK:
  ...
  <Step title="Fund + split — conditional_vault::split_tokens">
    ...
    -SOL holders the bond will later be minted to), then `split_tokens` splits its
    **own** SOL → equal pass-SOL + fail-SOL and its **own** USDC → equal pass-USDC +
    fail-USDC, to seed the pools. SDK: `futarchy.splitTokens` (`:222`).
  </Step>
  <Step title="Seed pass pool — amm::create_amm + amm::add_liquidity">
    `create_amm(base = pass-SOL, quote = pass-USDC)` then `add_liquidity`. SDK:
  ...
  <Step title="Seed fail pool — amm::create_amm + amm::add_liquidity">
    `create_amm(base = fail-SOL, quote = fail-USDC)` then `add_liquidity`.
  </Step>
...
```mermaid
flowchart TB
  Q[1 · initialize_question<br/>binary, resolver = oracle] --> KV[2 · init SOL vault<br/>pass-SOL / fail-SOL]
  Q --> UV[3 · init USDC vault<br/>pass-USDC / fail-USDC]
  KV --> SP[4 · split_tokens<br/>challenger SOL + USDC]
  UV --> SP
...
`PRICE_SCALE`, `MAX_OBSERVATION_CHANGE`, `DEFAULT_QUESTION_ID`): base 100 SOL (9 dp),
...
- **Vaults** — SOL vault underlying == `oracle.base_mint` (`:225-239`); USDC vault
...
- **Conditional mints** — pass/fail-SOL mints == `conditional_token_mint_pda(base_vault,
  0 / 1)` (`:261-265`).

It then program-splits the bond, escrows the challenger's USDC (sized via `spot_price`),
