...
Defined as `AMM_ID` in `programs/kassandra/src/cpi/metadao.rs:56-59`, and as
...
(`programs/kassandra/src/cpi/metadao.rs:100-107, 231-292`; SDK `ammV04.pda.*`).
...
```rust
// programs/kassandra/src/cpi/metadao.rs:78-90 (Borsh)
...
(`programs/kassandra/src/cpi/metadao.rs:131-178`; app decoder
...
<Note>
  ...
  **timestamp-weighted** version — `aggregator / (last_updated_ts − (created_at_ts +
  start_delay_seconds))` (`programs/kassandra/src/cpi/metadao_v06.rs:385-415`). That
