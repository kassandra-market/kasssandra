...
**v0.4.2** AMM: a **pass pool** trading pass-SOL against pass-USDC, and a **fail
pool** trading fail-SOL against fail-USDC. Kassandra stays on this AMM specifically
...
| --- | --- | --- | --- |
| **Pass** | pass-SOL | pass-USDC | USDC value of a unit of the bond **if the claim survives** |
| **Fail** | fail-SOL | fail-USDC | USDC value of a unit of the bond **if the claim is disqualified** |
...
The defaults are 100 SOL (9 dp) base and 100 USDC (6 dp) quote, which seeds an initial
...
<Info>
  ...
  is a `1e12`-scaled integer of **quote per base** (raw-USDC per raw-SOL, with the
...
<Note>
  The v0.6 futarchy spot oracle used by `spot_price` is the analogous but
