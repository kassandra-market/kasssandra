...
challenger opens a binary conditional market on the proposer's **SOL bond**
(pass-SOL vs fail-SOL, each priced in conditional USDC); traders move a
...
`programs/oracles/src/cpi/metadao_v06.rs` is the governance / `spot_price` layer,
...
<CardGroup cols={2}>
  <Card title="Proposer" icon="user-shield">
    Made the AI claim. Their locked SOL **bond** is the conditional collateral: it is
    split into idle pass-SOL + fail-SOL and is slashed if the market disqualifies
    them.
  </Card>
  <Card title="Challenger" icon="gavel">
    Opens the market and **escrows USDC** sized to the bond's value via `spot_price`.
    Seeds the pools and typically trades the fail side. Earns a SOL fee if the
    challenge succeeds.
  </Card>
...
The design is a **conditional-stake market**: pass-SOL and fail-SOL are fungible
...
<Steps>
  <Step title="Compose (challenger, off-chain)">
    ...
    question, the SOL and USDC conditional vaults, a `split`, and the two seeded
  ...
  <Step title="Open (open_challenge, Ix=4)">
    ...
    into idle pass/fail-SOL, escrows the challenger's USDC, and flips the claim to
  ...
  <Step title="Trade (swap)">
    ...
    fail-SOL price). See [Trading](/challenge/trading).
  </Step>
...
<CardGroup cols={2}>
  <Card title="Conditional vaults" icon="vault" href="/challenge/conditional-vaults">
    Splitting SOL and USDC into pass/fail conditional tokens, and redeeming on
    resolution.
  </Card>
