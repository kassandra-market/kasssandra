...
stack lives in `programs/kassandra/src/cpi/metadao.rs` (the v0.4 standalone
conditional vault + AMM). The separate v0.6 futarchy + Meteora stack in
`programs/kassandra/src/cpi/metadao_v06.rs` is the governance / `kass_price` layer,
...
(`programs/kassandra/src/processor/open_challenge.rs:15-18`). The challenge market is
