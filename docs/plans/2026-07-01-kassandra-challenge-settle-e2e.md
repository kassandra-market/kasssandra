...
### CS2 — surfpool `settle_challenge` E2E, both arms, REAL swap-driven v0.4 AMM TWAP (DONE — fully real)

Extended `sdk/test/surfpool/challenge-market-e2e.test.ts`: the T4 `fabricateAmmOwned` placeholders are replaced with **real** pass/fail v0.4 AMM pools and `settle_challenge` is driven end to end on forked mainnet for **both arms**, with the on-chain economics asserted over RPC. NO program/runner change.

- **Real pools (port `build_pool`):** `ammV04.createAmm` (initial obs = `quote·1e12/base`, `twapMaxObservationChangePerUpdate = MAX_PRICE = u64::MAX·1e12`, `start_delay = 0`) + `ammV04.addLiquidity` on this market's conditional-KASS/USDC mint pairs (PASS = idx0×idx0, FAIL = idx1×idx1). The CS1 pool/lp/vault-ATA seeds derive **correctly against the DEPLOYED `AMMyu265…` binary** — no divergence from the fixture, no stop-report. Pool liquidity is fabricated SPL balances at the payer's ATAs (the same input-materialization pattern as T4/G3).
- **DISQUALIFY arm:** PASS pool left neutral; a real `ammV04.swap` BUY (90 USDC) on the FAIL pool + two `ammV04.crankThatTwap` cranks ≥150 slots apart. Decoded over RPC: `pass_twap ≈ 1.0e9`, `fail_twap ≈ 2.40e9` → `fail·DEN (24.0e9) > pass·(DEN+NUM) (11.0e9)` clears the 10% margin **from real trading, not a seeded aggregator**. `settleChallenge` → question `[0,1]`; `kass_fee = bond/100` → challenger KASS; `bond − kass_fee` → `stake_vault` + `slashed_amount`/`bond_pool`; `surviving_count − 1`; full escrow → challenger; conditional KASS redeemed (holders burned, underlying drained). All asserted.
- **SURVIVE arm:** both pools cranked neutral (both TWAPs real + non-zero, margin holds). `settleChallenge` → question `[1,0]`; `usdc_fee = escrow/100` → proposer; remainder → challenger; bond redeemed un-slashed into `stake_vault` (`bond_pool` unchanged).

**The riskiest unknown — and the key finding (RESOLVED, not faked):** the v0.4 AMM crank is **slot-based** (`slot >= last_updated + ONE_MINUTE_IN_SLOTS(150)`), and `surfnet_timeTravel` moves only `getSlot`/`unix_timestamp` — **NOT** the `Clock::get()?.slot` the program reads during *execution* (diagnosed by decoding the live `Amm`: `last_updated == created_at`, `aggregator == 0` despite `getSlot` advancing ~950 slots). G3 worked only because the futarchy oracle is *timestamp*-based. Fix: boot this suite in surfpool's **`clock` block-production mode** with a fast `--slot-time` (10 ms) so the **execution** slot genuinely advances on a wall-clock timer; `advanceSlots` waits real slots between cranks. `harness.ts` gained opt-in `blockProductionMode` + `slotTimeMs` options (default unchanged → `transaction` + 0.4 s/slot, so T1-T3/G3 are untouched) and `advanceToUnix` is now slot-time-aware (the dispute-core time gates still jump via `timeTravel`).

Also fixed a **pre-existing** regression in this suite's `beforeAll`: after the G1-hardened `set_governance` landed, its stand-in random `dao_authority` failed the on-chain Squads-vault linkage check (`DaoAuthorityMismatch`, 0x20) — now derived via `futarchy.pda.squadsVault(squadsMultisig(kassDao), 0)`.

Results: default `pnpm test` green **101 offline** (untouched); gated `KASSANDRA_E2E=1 pnpm test:e2e` green **109 passed / 4 skipped** (the 4 skips = lifecycle + runner-mock arms, which need the runner binary; they skip cleanly). The challenge-market suite is **5/5** (programs-load, open-challenge, DISQUALIFY, SURVIVE). `pnpm typecheck` clean, `just build` clean.

**Covered-vs-deferred update:** `settle_challenge` (real swap-driven v0.4 AMM TWAP, both arms) is now **COVERED end-to-end** on forked MetaDAO. Only the **Meteora DAMM v2 spot path** (undeterminable zero-copy offsets) remains deferred.
