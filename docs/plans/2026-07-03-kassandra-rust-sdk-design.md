...
- `accounts` — zero-copy `bytemuck` decoders over the shared on-chain structs.
...
- **Runner** (`runner/src/submit.rs`): `program_id`, the `ai_claim` / `proposer` PDA
  derivations, and the `submit_ai_claim` instruction delegate to the SDK; local
  seed/system-id/account-meta boilerplate removed. Runner tests green.
