...
pnpm --filter @kassandra-market/markets test      # offline: parity + builders + litesvm
```
...
```ts
import { pda, metadao, flows, decodeMarket, MarketStatus } from "@kassandra-market/markets";
```
...
`sdks/oracles/rust` / program and **pinned by `test/parity.test.ts`** — a drift there fails
