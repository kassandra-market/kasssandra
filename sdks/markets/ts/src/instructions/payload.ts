 ...
 *
 * Every kassandra-market instruction's `data` is `[disc_byte, ...payload]` where
 * the payload mirrors the processor's exact byte layout (all integers
 ...
 * prepends the SINGLE-byte (Pinocchio) discriminant. The encodings match the
 * `*_at` / `to_le_bytes` reads in the Rust processors.
 */
