...
describe("parity guard: Ix discriminants (instruction.rs 0..=16)", () => {
  ...
  it("has exactly 17 instructions (0..=16), no more, no fewer", () => {
...
describe("parity guard: AccountType (state.rs 0..=5)", () => {
  const PINNED: Record<string, number> = {
    Uninitialized: 0,
    Config: 1,
    Market: 2,
...
describe("parity guard: MarketError (error.rs 0..=25)", () => {
...
// ── MetaDAO wire-format parity (sdks/oracles/rust/src/metadao.rs) ────────────────────────
...
describe("parity guard: MetaDAO instruction discriminators (sdks/oracles/rust/src/metadao.rs)", () => {
  // Hardcoded copies of the Anchor discriminators pinned in sdks/oracles/rust/src/metadao.rs.
  // crankThatTwap is NOT in sdks/oracles/rust (never composed Rust-side); pinned from the
...
describe("parity guard: MetaDAO arg encoders (byte-exact vs sdks/oracles/rust)", () => {
