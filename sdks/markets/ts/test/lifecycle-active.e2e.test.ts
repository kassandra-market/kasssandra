 ...
 *   1. initConfig(min_liquidity = 1 SOL, fee_bps = 100)
 ...
 *   4. a REAL swap grows the pool (a swapper splits SOL, sells cYES) so the LP
 ...
 *   7. collect_fee → fee_destination receives the accrued SOL cut, lpTotal is
 *      reduced, fee_collected == 1
 *   8. claim_lp for BOTH contributors pays pro-rata off the REDUCED lpTotal
 *   9. redeem → the YES holder is paid the winning leg 1:1 in SOL
 *
...
const SPLIT_AMT = 2_000_000_000n; // SOL the traded user splits for a position
...
describe("litesvm full active-market lifecycle (compose → activate → trade → resolve → collect_fee → claimLp → redeem)", () => {
  it("stands up, funds, activates, accrues + collects a protocol fee, then distributes reduced LP and pays a YES winner", async () => {
    ...
    // ── Stage 1: initConfig (fee_bps = 100, a SOL fee destination) ─────────
    const base = await ctx.createMint(9);
    const authority = (await ctx.fundedKeypair()).publicKey;
    const feeDestination = await ctx.createTokenAccount(base, authority, 0n);
    await ctx.sendOk(
      await initConfig({
    ...
    const creatorAta = await ctx.createTokenAccount(base, creator.publicKey, 5_000_000_000n);
    await ctx.sendOk(
      await createMarket({
        creator: creator.publicKey,
        oracle,
        baseMint: base,
        creatorBaseAta: creatorAta,
        seedAmount: SEED_A,
        outcomeIndex: 0,
      }),
      [creator],
    ...
    // A fresh swapper splits SWAP_SOL into cYES+cNO, then SELLS SWAP_IN cYES into
    ...
    const swKass = await ctx.createTokenAccount(base, swapper.publicKey, SWAP_SOL);
    ...
    const winKass = await ctx.createTokenAccount(base, winner.publicKey, SPLIT_AMT);
