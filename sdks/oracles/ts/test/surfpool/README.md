...
--block-production-mode transaction --no-deploy [--network mainnet | --offline]`, polls
...
| `gpt-oracle-e2e.test.ts` | Tracked GPT ELF + real `llm_oracle` against mock OpenRouter: `RequestAiOracle` CPI → callback → `ApplyExternalAiClaim`. Needs `LLM_ORACLE_BIN`. Port 8932 / WS 8933. Boots surfpool `--offline` so mainnet GPT PDAs are not fetched. |
